...
FLAME_MODEL_PATH = "assets/FLAME2023/generic_model.pkl"  # FLAME 2020
...
class FlameHead(nn.Module):
    ...
    def add_teeth(self):
        ...
        v_teeth_upper_edge -= torch.tensor([[0, 0, mean_dist]]) * 0.5 # slightly move the lower teeth to the back MINE
        
        v_teeth_upper_edge -= torch.tensor([[0, mean_dist, 0]]) * 1.7  # slightly move the upper teeth down MINE
        
        v_teeth_upper_root = v_teeth_upper_edge + torch.tensor([[0, mean_dist, 0]]) * 3  # * 1 scale the height of teeth MINE
        ...
        v_teeth_lower_edge -= torch.tensor([[0, 0, mean_dist]]) * 0.9  # slightly move the lower teeth to the back
    ...
    def forward(
        ...
        if return_landmarks:
            ...
            landmarks2d = vertices2landmarks(vertices, 
                                             self.faces,
                                             self.full_lmk_faces_idx,
                                             self.full_lmk_bary_coords)
            
            ret_vals.append(landmarks2d)
            ...
            landmarksmp = vertices2landmarks(vertices, self.faces,
                                    self.full_lmk_faces_idx.repeat(vertices.shape[0], 1),
                                    self.full_lmk_bary_coords.repeat(vertices.shape[0], 1, 1))
            ret_vals.append(landmarksmp)
