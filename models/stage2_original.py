...
class CodeTalker(BaseModel):
    def __init__(self, args):
        ...
        #self.audio_encoder = HubertModel.from_pretrained(args.wav2vec2model_path)
        ...
        # style embedding
        #self.learnable_style_emb = nn.Embedding(args.train_subjects_count, args.feature_dim)
        # Learnable styles bank: K codes of size [hidden_dim]
        self.style_codes = nn.Parameter(torch.randn(8, args.feature_dim))  # [K, D]
        self.style_proj  = nn.Linear(1024, 1024)  #

        print("Style codes shape: {}".format(self.style_codes.shape))
    ...
    def forward(self, audio_name, audio, audio_features, vertice, blendshapes, template, criterion):
        ...
        clip_repr = hidden_states.mean(dim=1)     # [B, 1024]
        style_emb = self.style_proj(clip_repr)    # [B, 1024]
        style_emb = style_emb.unsqueeze(1)        # [B, 1, 1024]
        ...
        vertice_input = self.vertice_map(vertice_blendshapes_input) + style_emb
        ...
        vertice_dec, blendshapes_out = torch.split(vertice_blendshapes_out, [15069,58], dim=2)
        vertice_out = vertice_dec + template

        # loss
        loss_vert = criterion(vertice_out, vertice) # (batch, seq_len, V*3)
        loss_blendshapes = criterion(blendshapes_out, blendshapes) # avoid comparing eyelashes
    ...
    def predict(self, audio, template, frame_num=None):
        ...
        for i in range(frame_num):
            if i==0:
                ...
                vertice_emb = vertice_emb + style_emb
            ...
            if i == 0:
                ...
                vertice_out_q, blendshapes_out_q = torch.split(vertice_blendshapes_out_q, [15069,58], dim=2)
        ...
        vertice_blendshapes_out = self.autoencoder.decode(feat_out_q)
        vertice_out, blendshapes_out= torch.split(vertice_blendshapes_out, [15069,58], dim=2)
