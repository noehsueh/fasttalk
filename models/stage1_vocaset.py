...
class VQAutoEncoder(BaseModel):
    ...
    def forward(self, vertice, blendshapes, template):
        ...
        return vertice_dec, blendshapes_dec, emb_loss, info
    ...
    def get_quant(self, vertice, blendshapes, x_a=None):
            combined = torch.cat((vertice, blendshapes), dim=-1)
            quant_z, _, info = self.encode(combined, x_a)
            indices = info[2]
            return quant_z, indices

    def get_quant_combined(self, combined, x_a=None):
            quant_z, _, info = self.encode(combined, x_a)
            indices = info[2]
            return quant_z, indices
